	...
	RuntimeEnvironment: function(Module, exposedLibs) {
		...
		var ASM_CONSTS = [
			...
			function($0) {
				...
				var audioDriverProcessCapture = cwrap("audio_driver_process_capture", null, [
					"number"
				]);
			...
			function($0, $1, $2) {
				...
				conn.setRemoteDescription({sdp: sdp, type: type})
					.then(onSet)
					.catch(onError);
			},
			function($0, $1, $2, $3) {
		...
		function emscriptenWebGLGetTexPixelData(
			...
			var sizePerPixel =
				__colorChannelsInGlTextureFormat[format] * __sizeOfGlTextureElementType[type];
			if (!sizePerPixel) {
				GL.recordError(1280);
				return;
			}
